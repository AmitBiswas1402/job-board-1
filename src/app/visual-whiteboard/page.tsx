    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans">
      ...
      <div className="vertex-container">
        <main className="flex-1 px-6 md:px-12 py-10 md:py-14 space-y-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-hairline pb-6 animate-vertex-fade-in">
            <div>
              <h1 className="vertex-heading text-2xl md:text-3xl flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="bg-surface-soft border border-hairline rounded-lg p-1 flex items-center gap-0.5">
                <button
                  onClick={() => setViewMode("kanban")}
                  className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-xs font-medium transition-all ${
                    viewMode === "kanban"
                      ? "bg-background text-foreground shadow-sm border border-hairline"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-vertex-slide-up">
            <div className="vertex-card p-5">
              ...
              <h3 className="text-3xl font-semibold text-foreground mt-2">{stats.Saved}</h3>
              <span className="text-[10px] font-medium text-muted-foreground mt-1 block">Awaiting your next move</span>
            </div>
            <div className="vertex-card p-5">
              <div className="flex items-center justify-between">
              ...
              <h3 className="text-3xl font-semibold text-blue-600 dark:text-blue-400 mt-2">{stats.Applied}</h3>
              <span className="text-[10px] font-medium text-muted-foreground mt-1 block">Out to employers</span>
            </div>
            <div className="vertex-card p-5">
              <div className="flex items-center justify-between">
              ...
              <h3 className="text-3xl font-semibold text-purple-600 dark:text-purple-400 mt-2">{stats.Interview}</h3>
              <span className="text-[10px] font-medium text-muted-foreground mt-1 block">Scheduled or in progress</span>
            </div>
            <div className="vertex-card p-5">
              <div className="flex items-center justify-between">
              ...
              <h3 className="text-3xl font-semibold text-emerald-600 dark:text-emerald-400 mt-2">{stats.Offer}</h3>
              <span className="text-[10px] font-medium text-muted-foreground mt-1 block">Celebrate & negotiate</span>
            </div>
          </div>
          ...
          {loading ? (
            ...
            /* Board Empty State */
            <div className="vertex-card p-14 text-center animate-vertex-slide-up-delay">
              <div className="mx-auto size-14 rounded-2xl bg-surface-soft border border-hairline flex items-center justify-center mb-5">
                <Inbox className="size-7 text-muted-foreground" />
              </div>
              <h3 className="text-lg font-semibold text-foreground">No applications tracked yet</h3>
              <p className="text-sm text-muted-foreground mt-2 max-w-md mx-auto leading-relaxed">
                Browse the job board and save a job to start building your pipeline. Once saved, manage every
                stage here — from applied to offer.
              </p>
              <a
                href="/jobs"
                className="vertex-cta inline-flex items-center gap-1.5 px-5 py-2.5 rounded-lg text-sm font-medium mt-6"
              >
                <Compass className="size-4" /> Browse Jobs
              </a>
            </div>
          ) : viewMode === "kanban" ? (
            /* Kanban Board View */
            <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
              <div className="flex gap-4 items-start select-none overflow-x-auto pb-4 board-scrollbar animate-vertex-slide-up-delay">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 w-full overflow-x-auto py-2">
                          <div className="flex items-center gap-2 min-w-[760px] relative justify-between">
                            ...
                            {STAGES.map((stage, idx) => {
                              ...
                              let stageColor = "border-hairline bg-canvas text-muted-foreground hover:border-foreground/25";
                              let lineColor = "bg-hairline";
                              
                              if (isCompleted) {
                                if (stage === "Rejected") {
                                  stageColor = isActive 
                                    ? "border-red-300 bg-red-500 text-white dark:border-red-500/40 dark:bg-red-500/20 dark:text-red-300" 
                                    : "border-red-200 bg-red-50/60 text-red-500 dark:border-red-500/20 dark:bg-red-500/10";
                                  lineColor = "bg-red-200 dark:bg-red-950/30";
                                } else if (stage === "Offer") {
                                  stageColor = isActive 
                                    ? "border-emerald-300 bg-emerald-500 text-white dark:border-emerald-500/40 dark:bg-emerald-500/20 dark:text-emerald-300" 
                                    : "border-emerald-200 bg-emerald-50/60 text-emerald-600 dark:border-emerald-500/20 dark:bg-emerald-500/10";
                                  lineColor = "bg-emerald-200 dark:bg-emerald-950/30";
                                } else {
                                  stageColor = isActive
                                    ? "border-primary bg-primary text-primary-foreground"
                                    : "border-primary/15 bg-surface-soft text-foreground/75";
                              ...
                              return (
                                <React.Fragment key={stage}>
                                  ...
                                  <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg border text-[10px] font-semibold transition-all ${stageColor} hover:scale-105 active:scale-95`}
                                  ...
                                  {idx < STAGES.length - 1 && (
                                    <div className={`h-[2px] flex-1 min-w-[20px] ${lineColor} rounded-full transition-all`} />
                                  )}
                                </React.Fragment>
                              );
                            })}
          ...
          <div className="relative w-full max-w-xl h-full bg-card border-l shadow-2xl flex flex-col z-10 animate-vertex-slide-up">
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 select-text">
              ...
              <div className="space-y-3">
                ...
                {selectedApp.resume ? (
                  <div className="flex justify-between items-center bg-surface-soft border border-hairline rounded-2xl p-4">
...
function KanbanColumn({ id, title, count, children }: KanbanColumnProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col w-[290px] shrink-0 bg-surface-soft border rounded-xl p-4 space-y-4 min-h-[500px] transition-all ${
        isOver
    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-3 min-h-[400px]">
        {count === 0 ? (
          <div className="flex-1 flex items-center justify-center text-center p-4 border border-dashed border-hairline rounded-lg">
            <span className="text-[10px] text-muted-foreground font-medium">Drag cards here</span>
          </div>
        ) : (
          children
        )}
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          <img
            ...
            className="size-9 rounded-lg object-contain bg-muted p-1.5 shrink-0 border"
            alt={app.job.company}
          />
        ...
        {/* Meta row: location + employment type */}
        <div className="flex items-center justify-between gap-2 text-[10px] text-muted-foreground">
          ...
          {app.job.employmentType && (
            <span className="shrink-0 font-medium text-foreground/70">{app.job.employmentType}</span>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 border-t border-hairline pt-2.5">
          {app.resume ? (
            <span className="text-[9px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 px-1.5 py-0.5 rounded flex items-center gap-0.5">
              Resume ✓
            </span>
          ) : (
            <span className="text-[9px] font-semibold text-muted-foreground bg-muted border px-1.5 py-0.5 rounded">
              No Resume
            </span>
          )}

          {app.atsReport ? (
            <span className="text-[9px] font-semibold text-foreground bg-muted border px-1.5 py-0.5 rounded flex items-center gap-0.5">
              ATS: {app.atsReport.score}%
            </span>
          ) : (
            <span className="text-[9px] font-semibold text-muted-foreground bg-muted border px-1.5 py-0.5 rounded">
          ...
          {app.coverLetter ? (
            <span className="text-[9px] font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 px-1.5 py-0.5 rounded flex items-center gap-0.5">
              Letter ✓
            </span>
          ) : (
            <span className="text-[9px] font-semibold text-muted-foreground bg-muted border px-1.5 py-0.5 rounded">
