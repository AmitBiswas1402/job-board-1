...
export default function IndustryInsightsPage() {
  ...
  const [employmentType] = useState("Full Time");
        ...
        <main className="flex-1 px-6 md:px-12 py-10 md:py-14 space-y-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-hairline pb-6 animate-vertex-fade-in">
            <div>
              <h1 className="vertex-heading flex items-center gap-2">
                <TrendingUp className="size-8 text-brand-accent" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              {insights?.latestNews ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {insights.latestNews.map((news, i) => (
                    <div
                      key={i}
                      className="vertex-card overflow-hidden flex flex-col group hover:shadow-md transition-all"
                    >
                      {/* Image header */}
                      <div className="h-44 w-full overflow-hidden relative border-b border-hairline">
                        <img
                          src={news.thumbnail}
                          onError={(e) => {
                        ...
                        <span className="absolute bottom-3 left-3 text-[9px] font-bold text-on-dark uppercase bg-surface-dark px-2 py-0.5 rounded border border-surface-dark-elevated font-mono">
                          {news.source}
                        </span>
                      </div>
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                        ...
                        <a
                          ...
                          className="w-full text-center py-2 border border-hairline hover:bg-surface-soft rounded-lg text-xs font-semibold text-ink flex items-center justify-center gap-1.5 transition-all"
                        >
                          Read Article
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-pulse">
                  {[1, 2, 3, 4].map((n) => (
                    <div key={n} className="border border-hairline bg-surface-card rounded-2xl h-72" />
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-brand-accent font-semibold font-mono">X/Twitter</span>
              </div>
              ...
              {insights?.trendingTweets ? (
                <div className="space-y-4">
                  {insights.trendingTweets.map((tweet, index) => (
                    <div
                      key={index}
                      className="vertex-card p-5 hover:border-hairline text-left space-y-3"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1">
                            <span className="text-xs font-bold text-ink font-heading">{tweet.author}</span>
                            <svg className="size-3 text-brand-accent fill-current" viewBox="0 0 24 24">
                      ...
                      <div className="flex items-center justify-between pt-2 text-muted-foreground border-t border-hairline">
                        ...
                        <button className="flex items-center gap-1.5 hover:text-brand-accent transition-colors text-[10px]">
                ...
                <div className="space-y-4 animate-pulse">
                  {[1, 2, 3].map((n) => (
                    <div key={n} className="border border-hairline bg-surface-card rounded-2xl h-36" />
                  ))}
                </div>
              )}
            </div>
          ...
          <div className="border-t border-hairline pt-12 space-y-8">
            ...
            <div className="max-w-xl">
              <h2 className="text-xl font-bold text-ink font-heading flex items-center gap-2">
            ...
            {/* Tech Search form */}
            <form onSubmit={handleSearchSubmit} className="max-w-4xl border border-hairline bg-surface-card p-5 rounded-2xl">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="w-full bg-primary hover:bg-primary-active text-on-primary rounded-lg font-semibold text-sm h-10 flex items-center justify-center gap-2 transition-all cursor-pointer border-0"
                >
                  {isPending ? (
                    <>
                      <div className="size-3.5 rounded-full border border-muted border-t-on-primary animate-spin" />
                      Analysing...
                    </>
                  ) : (
                    <>
              ...
              <div className="space-y-8 animate-vertex-slide-up">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="vertex-card p-6">
                    <h3 className="text-xs font-bold text-ink uppercase tracking-wider mb-5 flex items-center gap-2 font-heading">
                      <span className="size-1.5 rounded-full bg-brand-accent" />
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={insights.charts.salaryDistribution} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                          ...
                          <Tooltip
                            ...
                            itemStyle={{ color: "var(--ink)", fontSize: 11 }}
                          />
                          <Bar dataKey="salary" fill="var(--ink)" radius={[4, 4, 0, 0]}>
                            {insights.charts.salaryDistribution.map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={index === 2 ? "var(--ink)" : index === 1 ? "var(--brand-accent)" : "var(--muted)"} />
                            ))}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={insights.charts.hiringTrend} margin={{ top: 5, right: 10, left: -25, bottom: 5 }}>
                          ...
                          <Tooltip
                            contentStyle={{ backgroundColor: "var(--card)", borderColor: "var(--hairline)", borderRadius: "8px" }}
                            labelStyle={{ color: "var(--ink)", fontWeight: "bold", fontSize: 11 }}
                            itemStyle={{ color: "var(--brand-accent)", fontSize: 11 }}
                          />
                          <Line type="monotone" dataKey="openings" stroke="var(--brand-accent)" strokeWidth={2.5} dot={{ r: 3, stroke: "var(--brand-accent)", strokeWidth: 1 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                  ...
                  <div className="lg:col-span-5 vertex-card p-6">
                    <h3 className="text-xs font-bold text-ink uppercase tracking-wider mb-4 flex items-center gap-2 font-heading">
                      <span className="size-1.5 rounded-full bg-brand-accent" />
                      Ecosystem Keywords
                    </h3>
                  ...
                  <div className="lg:col-span-7 border border-success/20 bg-success/5 rounded-2xl p-6">
                    <h3 className="text-xs font-bold text-success uppercase tracking-wider mb-4 flex items-center gap-1.5">
                    ...
                    <div className="space-y-3 text-xs leading-relaxed text-ink/80">
                      {insights.marketSummary.map((point, index) => (
                        <div
                          key={index}
                          className="flex items-start gap-2.5 p-3 rounded-lg border border-hairline bg-surface-card"
                        >
